...
static rgbalist_t *psGetRgbalist(parser_t *ps) {
        if (ps == NULL) return NULL;
        return ps->rgblist;
}
...
static int psConsume(parser_t *ps, uint8_t type) {
        if (!psIsValid(ps)) return 0;
        if (psIsTokenType(ps, type)) {
                psNextToken(ps);
                return 1;
        }
...
static void psParseColorObject(parser_t *ps) {
        ...
        while (psIsValid(ps)) {
                ...
                if (!strncmp(key, STR_KEY_NAME, NAME_MAX_LEN)) {
                        ...
                        color.r = rgbaStrToByte(value);
                        color.g = rgbaStrToByte(value + 2);
                        color.b = rgbaStrToByte(value + 4);
                        color.isvc = 1;
                        printf("ho letto: #%s, ho scritto r: %d, g: %d, b: %d\n", value, color.r, color.g, color.b);
                }
                if (psConsume(ps, TOK_RBRACE)) break;
                psExpect(ps, TOK_COMMA);
                while (psConsume(ps, TOK_COMMA));
        }
        if (color.isvn && color.isvc) rgbalPush(psGetRgbalist(ps), &color);
